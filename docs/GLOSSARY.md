...
| **iogrid** | The founder's separate product (`iogrid/iogrid` repo) — a residential-bandwidth proxy mesh. vCard is iogrid's **first paying customer** (per PR #3, 2026-05-22). Outbound LinkedIn fetches tunnel through `proxy.iogrid.org:443` (SOCKS5 inside TLS). |
| **smoke-proxy** | The operator probe at `api/services/vcard-api/cmd/smoke-proxy/main.go`, invoked as `make smoke-proxy` from `api/`. GETs `https://api.ipify.org` through the iogrid client and fails non-zero if the proxied IP equals the local egress IP. The paired "loud" complement to LinkedIn-enrichment's graceful-skip contract — see [`PRINCIPLES.md`](PRINCIPLES.md) "Graceful-skip needs a loud smoke probe". |
